...
video_path    = "C:/Users/HP/Pictures/Camera Roll/WIN_20251128_20_00_11_Pro.mp4"
out_video     = "track2D/overlay6.mp4"
out_csv       = "track2D/track6.csv"
...
tracker_3D = tracker.Tracker3D(B=B, dispTolerance=0.2, angleTolerance=np.pi/4)
...
while True:
    ...
    for i, ball in enumerate(tracker_2D.balls):
        ...
        cv2.circle(rectL_half, center=(int(W_centroids * ball.position[1]), int(H_centroids * ball.position[0])),
                radius=ball.radius, color=(0, 255, 255), thickness=1) # Draw a yellow circle
        ...
        if posStar3D is not None:
            ...
            numPred = len(posStar3D)
            for j, (XStar, YStar, depthStar) in enumerate(posStar3D, start=1):
                ...
                cv2.circle(rectL_half, center=(center_xStar, center_yStar),
                           radius=np.clip(int(ball.radius*(depth/depthStar)), 1, a_max=30), color=(int(255/numPred * j), 0, int(255/numPred * j)),
                           thickness=1)  # Draw a purple circle
            # print()
...
# 2. Show the prediction made depthEstimationPeriod ago
