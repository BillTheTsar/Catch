...
video_path    = "C:/Users/HP/Pictures/Camera Roll/WIN_20260105_16_30_57_Pro.mp4"
out_video     = "track2D/overlay18.mp4"
out_csv       = "track2D/track18.csv"
...
tracker_3D = tracker.Tracker3D(B=B, dispTolerance=0.6, angleTolerance=np.pi/2)
eyes_2D = vision.Vision2D(H=H_full, W=W_full, B=B, padding=30)
...
while True:
    ...
    for i, ball in enumerate(tracker_2D.balls):
        ...
        cv2.putText(rectL_half, f"({X}, {Y}, {depth})",
                    ...
                    1,
                    2)
        # cv2.putText(rectL_half, f"dx: {np.round(ball.position[1]-0.5,2)}, dy: {np.round(ball.position[0]-0.5,2)}",
        #             (int(W_centroids * ball.position[1]), int(H_centroids * ball.position[0])+80),
        #             cv2.FONT_HERSHEY_SIMPLEX,
        #             1,
        #             (0, 0, 0),
        #             1,
