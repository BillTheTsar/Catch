...
video_path    = "C:/Users/HP/Pictures/Camera Roll/WIN_20251228_22_55_04_Pro.mp4"
out_video     = "track2D/predictOverlay14.mp4"
out_csv       = "track2D/predictTrack14.csv"
...
# For the prediction script ONLY
predictions = deque(maxlen=predictionGap)
for i in range(predictionGap):
    predictions.append([0,0,-1])
# End of prediction script ONLY
...
while True:
    ...
    for i, ball in enumerate(tracker_2D.balls):
        ...
        if frame_id % depthEstimationPeriod == 0:
            if ball.updated: # We saw the 2D ball this frame
                ...
                if ball3D.canPredict: # ball3D would not be None in this case
                    posStar3D = tracker_3D.predict_position_n_frames(ball3D, del_t, predictionGap) # Prediction in meters
                ...
                if ball3D.canPredict:
                    ...
                    posStar3D = tracker_3D.predict_position_n_frames(ball3D, del_t, predictionGap)
        ...
        if posStar3D is not None:
            ...
            predictions.append(posStar3D[-1]) # We push it from the end
            (XStar, YStar, depthStar) = predictions[0]
            ...
            if ((center_xStar < 0 or center_xStar > SCALE * W_full) or
                    (center_yStar < 0 or center_yStar > SCALE * H_full) or depthStar < 0 or YStar > cameraHeight):
                continue
            cv2.circle(rectL_half, center=(center_xStar, center_yStar),
                       radius=np.clip(int(ball.radius*(depth/depthStar)), 1, a_max=30), color=(255, 0, 255),
                       thickness=2)  # Draw a purple circle
            # print()
